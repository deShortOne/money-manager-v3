...
public sealed class BiWeeklyTests
{
    ...
    public void CalculateOverDueBillInfo_WithinOneWeekAfter_ReturnsOneIterationLate()
    {
        ...
        Assert.Equal(new OverDueBillInfo(9, 1, [new DateOnly(2024, 8, 15)]), threeDaysBeforeResult);

        var sixDaysBeforeResult = week.CalculateOverDueBill(-1, new DateOnly(2024, 8, 11), currentDay);
        Assert.Equal(new OverDueBillInfo(13, 1, [new DateOnly(2024, 8, 11)]), sixDaysBeforeResult);
    }
    ...
    public void CalculateOverDueBillInfo_OnTheDayOfTheSecondWeek_ReturnsOneIterationLate()
    {
        ...
        Assert.Equal(new OverDueBillInfo(14, 1, [new DateOnly(2024, 8, 10)]), fourteenDaysBeforeResult);
    }
    ...
    public void CalculateOverDueBillInfo_BetweenOneAndTwoWeeksAfter_ReturnsTwoIterationsLate()
    {
        ...
        Assert.Equal(new OverDueBillInfo(21, 2, [new DateOnly(2024, 8, 3), new DateOnly(2024, 8, 17)]), twentyOneDaysBeforeResult);

        var twentySevenDaysBeforeResult = week.CalculateOverDueBill(-1, new DateOnly(2024, 7, 28), currentDay);
        Assert.Equal(new OverDueBillInfo(27, 2, [new DateOnly(2024, 7, 28), new DateOnly(2024, 8, 11)]), twentySevenDaysBeforeResult);
    }
