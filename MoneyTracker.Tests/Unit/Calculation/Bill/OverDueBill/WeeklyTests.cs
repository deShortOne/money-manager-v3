...
public sealed class WeeklyTests
{
    ...
    public void CalculateOverDueBillInfo_WithinOneWeekAfter_ReturnsOneIterationLate()
    {
        ...
        Assert.Equal(new OverDueBillInfo(1, 1, [new DateOnly(2024, 8, 23)]), oneDayBeforeResult);

        var threeDaysBeforeResult = week.CalculateOverDueBill(-1, new DateOnly(2024, 8, 21), currentDay);
        Assert.Equal(new OverDueBillInfo(3, 1, [new DateOnly(2024, 8, 21)]), threeDaysBeforeResult);

        var sixDaysBeforeResult = week.CalculateOverDueBill(-1, new DateOnly(2024, 8, 18), currentDay);
        Assert.Equal(new OverDueBillInfo(6, 1, [new DateOnly(2024, 8, 18)]), sixDaysBeforeResult);
    }
    ...
    public void CalculateOverDueBillInfo_BetweenOneAndTwoWeeksAfter_ReturnsTwoIterationsLate()
    {
        ...
        Assert.Equal(new OverDueBillInfo(8, 2, [new DateOnly(2024, 8, 16), new DateOnly(2024, 8, 23)]), oneDayBeforeResult);

        var threeDaysBeforeResult = week.CalculateOverDueBill(-1, new DateOnly(2024, 8, 14), currentDay);
        Assert.Equal(new OverDueBillInfo(10, 2, [new DateOnly(2024, 8, 14), new DateOnly(2024, 8, 21)]), threeDaysBeforeResult);

        var sixDaysBeforeResult = week.CalculateOverDueBill(-1, new DateOnly(2024, 8, 11), currentDay);
        Assert.Equal(new OverDueBillInfo(13, 2, [new DateOnly(2024, 8, 11), new DateOnly(2024, 8, 18)]), sixDaysBeforeResult);
    }
