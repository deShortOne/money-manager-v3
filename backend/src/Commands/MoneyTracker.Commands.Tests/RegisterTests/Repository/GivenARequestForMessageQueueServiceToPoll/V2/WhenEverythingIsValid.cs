...
public class WhenEverythingIsValid : MessageQueueServiceHelper
{
    ...
    private readonly int _payerId = 24;
    private readonly AccountUserEntity _payeeAccountUserEntity;
    private readonly AccountUserEntity _payerAccountUserEntity;
    ...
    public WhenEverythingIsValid()
    {
        _payeeAccountUserEntity = new AccountUserEntity(_payeeId, 2894, _userId, true);
        _payerAccountUserEntity = new AccountUserEntity(_payerId, 52, _userId, true);
    }
    ...
    public override async Task InitializeAsync()
    {
        ...
        _mockAccountCommandRepository
            .Setup(x => x.GetAccountUserEntity(_payer, _userId, CancellationToken.None))
            .ReturnsAsync(_payerAccountUserEntity);
    ...
    public void ThenTheTemporaryTransactionIsMappedCorrectly()
    {
        Assert.Multiple(() =>
        {
            ...
            Assert.Equal(_datePaid, _resultTemporaryTransaction.DatePaid);
            Assert.Equal(_payeeId, _resultTemporaryTransaction.PayeeId);
            Assert.Equal(_payerId, _resultTemporaryTransaction.PayerId);
        });
    }
